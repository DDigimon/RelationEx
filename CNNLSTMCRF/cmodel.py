...
class SequenceLabelingModel(object):
    ...
    def __init__(self, sequence_length,word_length, nb_classes, nb_hidden=512, feature_names=None,
                 feature_init_weight_dict=None, feature_weight_shape_dict=None,
                 feature_weight_dropout_dict=None, char_init_weight_dict=None,
                 char_weight_dropout_dict=None,char_weight_shape_dict=None,
                 dropout_rate=0., use_crf=True,
                 filter_size=2,char_embedding_size=64,num_filter=32,
                 path_model=None, nb_epoch=200, batch_size=128, train_max_patience=10,
                 l2_rate=0.01, rnn_unit='lstm', learning_rate=0.001, max_char=2,clip=None):
        """
        Args:
        ...
        self.char_init_weight_dict=char_init_weight_dict
        self.feature_weight_shape_dict = feature_weight_shape_dict
        self.char_weight_shape_dict=char_weight_shape_dict
        self.feature_weight_dropout_dict = feature_weight_dropout_dict
        self.char_weight_dropout_dict=char_weight_dropout_dict
    ...
    def fit(self, data_dict, dev_size=0.2, seed=1337):
        ...
        for step in range(self.nb_epoch):
            ...
            data_list.append(data_train_dict['char'])
            ...
            for i in tqdm(range(nb_train)):
                ...
                for feature_name in self.feature_names:  # features
                    ...
                    feed_dict.update(item)
                # 喂入字向量
                feed_dict.update({self.input_char_ph_dict['char']:data_train_dict['char'][batch_indices]})
                feed_dict.update({self.weight_dropout_ph_dict['char']:self.char_weight_dropout_dict['char']})
    ...
    def evaluate(self, data_dict):
        ...
        for i in range(nb_eval):
            ...
            # 喂入字向量
            feed_dict.update({self.input_char_ph_dict['char']: data_dict['char'][batch_indices]})
            feed_dict.update({self.weight_dropout_ph_dict['char']: 0.})
            # label feed
    ...
    def predict(self, data_test_dict):
        ...
        for i in tqdm(range(nb_test)):
            ...
            for feature_name in self.feature_names:  # features and dropout
                ...
                feed_dict.update(item)
            feed_dict.update({self.dropout_rate_ph: 0.})
            feed_dict.update({self.input_char_ph_dict['char']: data_test_dict['char'][batch_indices]})
            feed_dict.update({self.weight_dropout_ph_dict['char']: 0.})
