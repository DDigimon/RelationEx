...
def _embedding_dim(config):
    embed_dim=0
    features=config['model_params']['feature_names']
    for f in features:
        embed_dim+=int(config['model_params']['embed_params'][f]['shape'][1])
    return embed_dim
def main():
    # 加载配置文件
    with open(my_config) as file_config:
    ...
    # 加载数据
    embed_dim = _embedding_dim(config)
    # 加载vocs
    path_vocs = []
    for feature_name in feature_names:
    ...
    model = SequenceLabelingModel(
        ...
        path_model=config['model_params']['path_model'],embed_dim=embed_dim)
